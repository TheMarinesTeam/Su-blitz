...
click[0].onclick = function() {
    if (inputs[0].value.length <= 6){
        error_notif[0].textContent = "يجب ان يكون الاسم اكبر من ست احرف"
    }else{
        error_notif[0].textContent = ""
    }
    if (isNaN(inputs[0].value.charAt(0)) === false){
        error_notif[1].textContent = "اول خانه لا يجب ان تكون رقم"
    }else{
        error_notif[1].textContent = ""
    }
    if(inputs[1].value.length <= 8){
        error_notif[3].innerHTML = "يجب ان تكون كلمه المرور اكثر من ثمان احرف"
    }else{
